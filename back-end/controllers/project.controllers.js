...
export const updateProject = async (req, res) => {
    try{
        ...
        if(req.user.id !== foundProject.researcherId.toString()){
            return res.status(403).json({ message: "Can only update your own projects" });
        }
        ...
        return res.status(200).json({ message: "Project was updated", project: updatedProject });
    }catch(err){
...
export const deleteProject = async (req, res) => {
    ...
    if(req.user.id !== foundProject.researcherId.toString()){
        return res.status(403).json({ message: "Can't delete other researchers projects"});
    }
    ...
    return res.status(200).json({ message: "Project with associated questions, was deleted." });
    }catch(err){
        return res.status(500).json({ message: "Server error", error: err.message});
    }
}
